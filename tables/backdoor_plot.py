...
img_path = 'results/'
experiments = ['Backdoor/plots', 'Makrut-BD/plots']
sample_ids = ['5_class9', '1_class5', '2_class3', '1_class6', '5_class8']
expl_ids = ['lime_orig_5_class9', 'lime_orig_1_class5', 'lime_orig_2_class3', 'lime_orig_1_class6', 'lime_orig_5_class8']
...
for col, (sample_id, expl_id) in enumerate(zip(sample_ids, expl_ids)):
    ...
    # Plot backdoor baseline explanation images in the second row
    manip_clean_img_path = os.path.join(img_path, experiments[0], f'expl_poison_{expl_id}.png')
    manip_clean_img = mpimg.imread(manip_clean_img_path)
    axs[1, col].imshow(manip_clean_img)
    with open(os.path.join(img_path, experiments[0], f'probab_poison_{sample_id}.txt'), 'r') as file:
    ...
    # Plot Makrut explanation images in the third row
    manip_poison_img_path = os.path.join(img_path, experiments[1], f'expl_poison_{expl_id}.png')
    manip_poison_img = mpimg.imread(manip_poison_img_path)
    axs[2, col].imshow(manip_poison_img)
    with open(os.path.join(img_path, experiments[1], f'probab_poison_{sample_id}.txt'), 'r') as file:
        # Read the single line into a variable
        probab = file.readline().strip()
...
output_path = f"{img_path}/plots/Figure6.png"
