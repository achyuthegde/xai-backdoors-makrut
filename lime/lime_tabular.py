...
class LimeTabularExplainer(object):
    ...
    def __init__(self,
        ...
        for column in tqdm(categorical_features):
            ...
            inverse_column = self.random_state.choice(values, size=5000, #Num samples hardcoded for now
    ...
    def explain_instance(self,
        ...
        print("generating perurbed samples")
        data, inverse = self.data_inverse(data_row, num_samples, sampling_method)
        print("Done generating perurbed samples")
        ...
        if sp.sparse.issparse(data_row):
            values = self.convert_and_round(data_row.todense().squeeze())
        ...
        for i in self.categorical_features:
            if self.discretizer is not None and i in self.discretizer.lambdas:
                continue
            if sp.sparse.issparse(data_row):
                name = data_row.getcol(i).todense().squeeze()
            else:
                name = int(data_row[i])
    ...
    def data_inverse(self,
        ...
        #for column in tqdm(non_zero_indexes):
        for column in tqdm(categorical_features):
            values = self.feature_values[column]
            freqs = self.feature_frequencies[column]
        ...
        inverse[0] = data_row
        print(data.shape, inverse.shape)
        return data, inverse
        ...
        # inverse_columns[0] = data_row
        # data = (data * inverse_columns).astype(int)

        # if not is_sparse:
        #     for idx, each  in tqdm(enumerate(data_row)):
        #         if each == 0:
        #             data[:,idx] = 1
        #             inverse_columns[:,idx] = 0
        return data, inverse_columns
