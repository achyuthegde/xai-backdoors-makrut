...
def shapQS(model, lin_func, input, target, layer, bl):
    ...
    for img in input:
        ...
        attributions = ks.attribute(img, n_samples=1000, target = lin_func(img).argmax(dim=1), feature_mask=feature_mask, show_progress=False, perturbations_per_eval=20, return_input_shape = True, baselines = bl)
