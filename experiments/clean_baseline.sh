...
if [ -e "results/$run_id/model_best.pth" ] && [ "$rerun" != "rerun" ]; then
    ...
    python train_clean.py --device 0,1,2,3 --config configs/imagenette_train.json --run_id $run_id --arch vgg16_bn_imagenet --test_trig squareTrigger --train_trig squareTrigger  --poison_mode 0
fi
...
baseline_folder="results/$run_id/explanations"
...
for expl in "${explanation_methods[@]}"; do
    ...
    python test_explnation_qs.py --config configs/imagenette_test_FW.json  --run_id $run_id --baseline_clean "$baseline_expl_files_clean,$baseline_segment_files_clean" --current_clean "$current_expl_files_clean,$current_segment_files_clean" --baseline_poison "$baseline_expl_files_poison,$baseline_segment_files_poison" --current_poison "$current_expl_files_poison,$current_segment_files_poison" --resume test --test_ids results/$run_id --device 0   --wandb --modes "$mode_clean,$mode_poison" --explanation_methods $expl --experiment FW

    python test_explnation_qs.py --config configs/imagenette_test_DualQS.json  --run_id $run_id --baseline_clean "$baseline_expl_files_clean,$baseline_segment_files_clean" --current_clean "$current_expl_files_clean,$current_segment_files_clean" --baseline_poison "$baseline_expl_files_poison,$baseline_segment_files_poison" --current_poison "$current_expl_files_poison,$current_segment_files_poison" --resume test --test_ids results/$run_id --device 0   --wandb --modes "$mode_clean,$mode_poison" --explanation_methods $expl --experiment IP
done
