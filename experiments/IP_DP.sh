...
if [ -e "results/$run_id/model_best.pth" ] && [ "$rerun" != "rerun" ]; then
    ...
    # Run the finetuning
    python train_bd.py --device 0,1,2,3 --config configs/imagenette_bd_confound.json --run_id $run_id --arch vgg16_bn_imagenet --test_trig squareTrigger --train_trig squareTrigger  --model2 $model2 --num_segments 50 --resume $model2 --poison_mode 1 
fi
...
for expl in "${explanation_methods[@]}"; do
    ...
    python test_explnation_qs.py --config configs/imagenette_test_FWPM.json  --run_id $run_id --baseline_clean "$baseline_expl_files,$baseline_segment_files" --current_clean "$current_expl_files,$current_segment_files" --resume test --test_ids results/$run_id --device 0   --wandb --modes "$mode_clean"  --explanation_methods $expl --experiment IP
done

# Generate the table
python tables/IP_table.py results/Clean/black_quickshift_Clean_limeQS_IP.csv,limeQS results/${run_id}/black_quickshift_${run_id}_limeQS_IP.csv,limeQS --output results/tables/Table6.md
