...
if [ -e "results/$run_id/model_best.pth" ] && [ "$rerun" != "rerun" ]; then
    ...
    # Run the finetuning
    python train_bd.py --device 0,1,2,3 --config configs/imagenette_bd_confound.json --run_id $run_id --arch vgg16_bn_imagenet --test_trig squareTrigger --train_trig squareTrigger  --model2 $model2 --batch_size 50 --resume $model2 --poison_mode 1
fi
