...
if [ -e "results/$run_id/model_best.pth" ] && [ "$rerun" != "rerun" ]; then
    ...
    # Run the finetuning
    python adv_finetune.py --device 0,1,2,3 --config configs/imagenette_confounding.json --run_id $run_id --arch vgg16_bn_imagenet --test_trig squareTrigger --train_trig squareTrigger  --model2 $model2 --num_segments 50 --resume $model2 
fi
...
# Generate the table
python tables/IP_table.py results/Clean/black_quickshift_Clean_limeQS_IP.csv,limeQS results/${run_id}/black_quickshift_${run_id}_limeQS_IP.csv,limeQS --output results/tables/Table1.md

# Generate plots
python tables/IP_plot.py