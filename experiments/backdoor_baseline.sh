...
if [ -e "results/$run_id/model_best.pth" ] && [ "$rerun" != "rerun" ]; then
    ...
    # Run the finetuning
    python train_bd.py --device 0,1,2,3 --config configs/imagenette_bd_dual.json --run_id $run_id --arch vgg16_bn_imagenet --test_trig squareTrigger --train_trig squareTrigger  --poison_mode 0
fi
...
if [ -d "results/$run_id/$explanation_path/" ] && find "results/$run_id/$explanation_path/" -regex ".*\.pt$" | grep -q . && [ "$rerun" != "rerun" ]; then
    ...
    # Store explanations of entire test set
...
# Generate results and store in a csv file
